...
-- seems like we have only one create_date...
...
-- select film_id, title, description,length, rental_rate from film where title = 'Sunset Racer';
...
-- select film_id, title, description,length, rental_rate from film where title LIKE 'Su%';

-- 10. Write a query which will find the 10 cheapest movies.
-- select title, rental_rate from film order by rental_rate asc fetch first 10 rows only;

-- 11. Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
-- Bonus: Try to not use LIMIT.
-- select title, rental_rate from film order by rental_rate asc limit 10 offset 9;
...
-- with info_table as (select 
-- c.customer_id, c.first_name, c.last_name, p.amount, p.payment_date, p.staff_id
-- from customer as c
-- join payment as p
-- on c.customer_id = p.customer_id)

-- select it.*, s.staff_id
-- from info_table as it
-- join staff as s on it.staff_id = s.staff_id order by s.staff_id;